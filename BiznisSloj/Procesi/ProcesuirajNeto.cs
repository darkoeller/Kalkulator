...
namespace BiznisSloj.Procesi
{
    public class ProcesuirajNeto
    {
        ...
        private void PostaviKoeficijentePorezaPrireza()
        {
            ...
            var koeficijenti = new IzracunPoreznihKoeficijenata(Prirez);            
            KoefPrireza = koeficijenti.PoreznoPrirezniKoef;
            KoefPorezaPrireza24 = koeficijenti.KoeficijentPoreza24;
            KoefPorezaPrireza36 = koeficijenti.KoeficijentPoreza36;
        }
        ...
        private decimal NadjiMetoduZaIzracun(decimal neto, decimal odbitak)
        {
            ...
            if (neto < (30000.0m - (7200.0m * KoefPrireza) + odbitak))
            return Math.Round(DrugaMetoda(neto, odbitak), 2);
            //sa ograničenjem doprinosa
            if (neto > (30000.0m - (7200.0m * KoefPrireza)+ odbitak ) || neto < 40550.0m - (7200.0m * KoefPrireza + (10500.0m - odbitak) * KoefPrireza))
        ...
        private decimal PetaMetoda(decimal neto, decimal odbitak) => 30000.0m + odbitak + (neto - (30000.0m - (7200.0m * KoefPrireza)+ odbitak)) * KoefPorezaPrireza36 + 10137.6m;
        //bez ograničenja
        //private decimal CetvrtaMetoda(decimal neto, decimal odbitak) => ((30000.0m + odbitak + (neto - (30000.0m - (7200.0m * KoefPrireza) + odbitak)) * KoefPorezaPrireza36)/ 0.8m);
        private decimal DrugaMetoda(decimal neto, decimal odbitak) => ((((neto - odbitak) * KoefPorezaPrireza24)) + odbitak) / 0.8m;
    }
}